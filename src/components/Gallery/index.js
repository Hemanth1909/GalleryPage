...
class Gallery extends Component {
  ...
  selectedImage = () => {
    ...
    const selectedImage = imagesList.filter(
      eachImage => selectedImageId === eachImage.id,
    )
    ...
    return (
      <div className="all-back">
        ...
        <div className="bottom-container">
          <h1 className="bottom-heading">Nature Photography</h1>
          <p className="second-heading">Nature Photography by Hemanth</p>
          <ul className="thumbnail-container">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                imageDetails={eachImage}
                clickImage={this.clickImage}
                isActive={selectedImageId === eachImage.id}
              />
            ))}
          </ul>
        </div>
